...
tpm_B73_top <- tpm_fit_B73 %>% filter(adj.P.Val <= 0.05) %>% arrange(P.Value)
dim(tpm_B73_top)
head(tpm_B73_top)
```
...
tpm_TIL01_top <- tpm_fit_TIL01 %>% filter(adj.P.Val <= 0.05) %>% arrange(P.Value)
dim(tpm_TIL01_top)
head(tpm_TIL01_top)
```
...
tpm_results %>% ggplot() + geom_point(aes(x=logFC.x, y=logFC.y)) + 
  xlab("Differentially expressed from B73") + 
  ylab("Differentially expressed from TIL01")
```
...
```{r}
lfc <- tpm_results %>% select(gene, logFC.x, logFC.y, P.Value.x, P.Value.y, adj.P.Val.x, adj.P.Val.y) %>% arrange(desc(logFC.x))
