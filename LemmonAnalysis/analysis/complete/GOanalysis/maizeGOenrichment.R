...
setwd("/Users/caryn/Box Sync/Projects/Highland_GeneticIncompatibility/LemmonAnalysis/analysis/complete/")
...
sig_genes <- readRDS("bygroup_sig.rds")

sig_genes %>% ggplot() + geom_point(aes(x=logFC.x, y=logFC.y, color=as.factor(de)))
sig_genes %>% filter(de=="both") %>% ggplot() + geom_point(aes(x=logFC.x, y=logFC.y, color=as.factor(group)), alpha = 0.5)

#Ngenes <- sig_genes %>% filter(de=="both") %>% select(gene)
#Ngenes <- unique(Ngenes)

gene_universe <- sig_genes %>% filter(de=="both") %>% select(gene)
gene_universe <- as.character(gene_universe$gene)
